...
import { logCheckinAction, type NcActionState } from "../actions/new-converts-actions";
import { CALL_STATUS_OPTIONS } from "@/features/call-pipeline/constants";

const initialState: NcActionState & { success?: boolean } = { error: null };
...
export function CheckinDialog({
  open, onOpenChange, ncId, ncName, month, callerName, existing, onLogged,
}: {
  open: boolean; onOpenChange: (open: boolean) => void; ncId: string; ncName: string;
  month: number; callerName: string; existing?: ExistingCheckin; onLogged: () => void;
}) {
  const isEditMode = !!existing;
  const action = logCheckinAction.bind(null, ncId, month, callerName);
  const [state, formAction, pending] = useActionState(action, initialState);
  const [flagged, setFlagged] = useState(existing?.flagged_for_pastoral ?? false);

  useEffect(() => {
    if (state.success) {
      toast.success(`Month ${month} ${isEditMode ? "updated" : "logged"} for ${ncName}.`);
      onLogged();
      onOpenChange(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>{isEditMode ? "Edit" : ""} Month {month} Check-In — {ncName}</DialogTitle></DialogHeader>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <Label>Notes</Label>
            <Textarea name="notes" rows={3} placeholder="How is their walk with Christ progressing?" defaultValue={existing?.notes ?? ""} />
          </div>
          ...
          <Button type="submit" disabled={pending} className="w-full">
            ...
            {pending ? "Saving…" : <><CheckCircle2 size={14} /> {isEditMode ? "Update" : "Save"} Month {month}</>}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
