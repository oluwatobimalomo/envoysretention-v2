...
import { assignNcAction, bulkAssignNcAction, unassignNcAction, exportNewConvertsCsvAction } from "../actions/new-converts-actions";
...
type Filter = "unassigned" | "assigned" | "completed";

export function AssignNcClient({ rows, teamMembers }: { rows: EnrichedNewConvert[]; teamMembers: { id: string; full_name: string }[] }) {
  ...
  const filtered = rows.filter((r) => {
    ...
    if (filter === "completed") return ncComplete(r.fbRows) && r.envoys_training_completed;
    return true;
  });
  ...
  const bulkAssign = () => {
    ...
    if (!ids.length) { toast.error("No unassigned New Converts."); return; }
    startTransition(async () => {
      await bulkAssignNcAction(ids, bulkMember);
  ...
  const assignOne = (id: string, memberId: string) => {
    startTransition(async () => {
      await assignNcAction(id, memberId);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(["unassigned", "assigned", "completed"] as Filter[]).map((f) => (
      ...
      <div className="space-y-2">
        {filtered.length === 0 && <div className="rounded-xl border border-dashed py-12 text-center text-sm text-muted-foreground">No New Converts in this category.</div>}
        {filtered.map((r) => (
          <div key={r.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-card p-4">
            <div>
              <p className="font-medium">{r.full_name}{ncGenderTag(r.gender)}</p>
              <p className="text-xs text-muted-foreground">{r.phone} · {r.conversion_type}</p>
            </div>
            <AssignmentControl
