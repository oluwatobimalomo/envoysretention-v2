...
export function PeLogFeedbackDialog({
  open, onOpenChange, peId, peName, week, callerName, onLogged,
}: {
  open: boolean; onOpenChange: (open: boolean) => void; peId: string; peName: string;
  week: number; callerName: string; onLogged: () => void;
}) {
  ...
  useEffect(() => {
    if (state.success) {
      toast.success(`Week ${week} logged for ${peName}.`);
      onLogged();
      onOpenChange(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Week {week} Check-In — {peName}</DialogTitle></DialogHeader>
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <Label>Call Status</Label>
            <NativeSelect name="call_status" required defaultValue="">
          ...
          <div className="space-y-1.5">
            <Label>Notes</Label>
            <Textarea name="notes" rows={3} placeholder="How are they settling in?" />
          </div>
          <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-4 space-y-3">
            ...
            {flagged && <Textarea name="flag_reason" rows={2} placeholder="Reason…" />}
          </div>
          {state.error && <p className="rounded-md bg-destructive/10 px-3 py-2 text-sm text-destructive">{state.error}</p>}
          <Button type="submit" disabled={pending} className="w-full">
            {pending && <Loader2 className="animate-spin" />}
            {pending ? "Saving…" : <><CheckCircle2 size={14} /> Save Week {week}</>}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
