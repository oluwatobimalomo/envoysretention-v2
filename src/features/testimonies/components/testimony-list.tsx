...
import { exportTestimoniesCsvAction } from "../actions/testimonies-actions";
import { downloadCsv } from "@/lib/csv";
import type { TestimonyEntry } from "../constants";

export function TestimonyList({ entries, filename }: { entries: TestimonyEntry[]; filename: string }) {
  ...
  const toggleAll = () => setSelected((prev) => (prev.size === entries.length ? new Set() : new Set(entries.map((e) => e.id))));

  const handleExport = async () => {
    const toExport = entries.filter((e) => selected.has(e.id));
    if (toExport.length === 0) { toast.error("Select at least one testimony first."); return; }
    setExporting(true);
    try {
      const csv = await exportTestimoniesCsvAction(toExport);
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed py-16 text-center">
        <BookHeart className="text-muted-foreground" size={28} />
        <p className="font-medium">No testimonies yet</p>
        <p className="text-sm text-muted-foreground">They&apos;ll show up here as they come in.</p>
      </div>
    );
  }
          ...
          <label key={e.id} className="flex cursor-pointer items-start gap-3 rounded-xl border bg-card p-4 hover:bg-accent/30">
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-1.5 flex flex-wrap items-center gap-2">
                ...
                {e.category && <Badge variant="gold">{e.category}</Badge>}
                <span className="text-xs text-muted-foreground">{e.date}</span>
              </div>
              <p className="text-sm text-muted-foreground break-words">{e.testimony}</p>
            </div>
          </label>
        ))}
      </div>
